...
const SERVICE_WORKER_JS_PATH = () => process.env.TEZ_DEVELOPMENT == 1 ? common.getPath([process.cwd(), "packages", "vite", "templates", "service-worker.ts"], false) : common.getPath([common.commonContainer.buildOptions.rootDir, "node_modules", "@tezjs", "vite", "templates", "service-worker.ts"], false);
const TZ_JS_PATH = () => process.env.TEZ_DEVELOPMENT == 1 ? common.getPath([process.cwd(), "packages", "vite", "templates", "tz-web-worker.js"], false) : common.getPath([common.commonContainer.buildOptions.rootDir, "node_modules", "@tezjs", "vite", "templates", "tz-web-worker.js"], false);
      ...
      for (const slotName of slots) {
        ...
        for (const componentItem of components) {
          ...
          if (slotName === "default" && isPageSlot && itemCount >= common.commonContainer.tezConfig.payload.page.maxPreLoadComponent) {
            ...
            if (!isPageSlot && componentsCount > 0 && common.commonContainer.tezConfig.payload.page.maxPreLoadComponent > componentsCount) {
...
var https$1 = require$$2__default;
var Writable = require$$3__default.Writable;
var assert = require$$4__default;
var debug = debug_1;
...
const indexTemplate = (seo) => {
  let imageLoader = "";
  if (common.commonContainer.tezConfig?.client?.loaderImage) {
    imageLoader = `<img src="${common.commonContainer.tezConfig?.client?.loaderImage}" style="position:absolute;top:20%;left:36%; margin:0 auto;" />`;
  }
  return `<!DOCTYPE html>
...
function getAddLib(isAddLib) {
  let codes = new Array();
  if (common.commonContainer.tezConfig.pwa || isAddLib) {
...
function registerServiceWorker() {
  let code = "";
  if (common.commonContainer.buildOptions.commandName === "build" && common.commonContainer.tezConfig.pwa) {
    code = `
...
const appContainer = new class {
  ...
  getProps(existsFilesorFolders) {
    let props = "";
    if (common.commonContainer.tezConfig.client)
      props += `clientConfig: ${JSON.stringify(common.commonContainer.tezConfig.client)},`;
...
const preCacheAssets = (assets) => {
  return `
    window.TEZ_PRE_CACHE_ASSETS={
        cacheName:"${common.commonContainer.buildOptions.buildVersion}",
...
class HtmlGen {
  ...
  async build() {
    ...
    for (var route of this.routes) {
      ...
      let page = {
        head: {
          metaTag: {},
          inlineStyle: common.commonContainer.tezConfig.build.inLineCss ? this.getInlineCss(path) : new Array(),
  ...
  async getInlineFontCss() {
    ...
    if (common.commonContainer.tezConfig.build?.inlineFontCss) {
      const uris = common.commonContainer.tezConfig.build?.inlineFontCss;
      for (const cssUri of uris) {
        ...
        if (data) {
          ...
          for (let uri of uris2) {
            ...
            common.writeFileSync(this.commonPathResolver.getFontFilePath(fileName), data2, true);
          }
  ...
  addServiceWrokerDeps(page, jsCodeGen) {
    if (common.commonContainer.tezConfig.pwa) {
      let pwaConfig = common.commonContainer.tezConfig.pwa;
  ...
  getPreloads(path, jsCodeGen) {
    ...
    if (common.commonContainer.tezConfig.htmlMeta?.head?.handler?.preloads) {
      let customPreloads = common.commonContainer.tezConfig.htmlMeta?.head?.handler?.preloads(path, jsCodeGen.components);
...
function replacer(code) {
  code = code.replace(SERVICE_WORKER_CACHE_VERSION, common.commonContainer.buildOptions.buildVersion.toString());
  return code;
}
...
function getPwaConfig() {
  if (common.commonContainer.tezConfig.pwaConfigPath) {
    let configPath = common.getPath([common.commonContainer.buildOptions.rootDir, common.commonContainer.tezConfig.pwaConfigPath]);
    return getImport(configPath.split("\\").join("/"));
  }
  return `const pwaConfig = {default:()=>{ return ${JSON.stringify(common.commonContainer.tezConfig.pwa)}; }}`;
}
function addManifestJson(pwaConfig, commonPath) {
  if (pwaConfig.config)
    common.writeFileSync(commonPath.manifestJsonPath, pwaConfig.config);
}
function tezPWA() {
  return {
    name: "vite:tez-pwa",
    async generateBundle(_outputOptions) {
      if (common.commonContainer.tezConfig.pwaConfigPath || common.commonContainer.tezConfig.pwa) {
        let commonPathResolver = new common.CommonPathResolver();
        const serviceWorkerTemplatePath = SERVICE_WORKER_JS_PATH();
        if (commonPathResolver.pathExists(serviceWorkerTemplatePath)) {
          let serviceWorkerCode = common.readFileSync(serviceWorkerTemplatePath, true);
          serviceWorkerCode = serviceWorkerCode.replace(IMPORT_SCRIPT_SERVICE_WORKER, getPwaConfig());
          common.writeFileSync(commonPathResolver.serviceWorkerGenPath, serviceWorkerCode, true);
          const result = await esbuild.build({
          ...
          for (const output of result.outputFiles) {
            ...
            common.writeFileSync(commonPathResolver.serviceWorkerJsPath, code, true);
          }
          commonPathResolver.deleteFile(commonPathResolver.serviceWorkerGenPath);
        }
        addManifestJson(common.commonContainer.tezConfig.pwa, commonPathResolver);
      }
    }
  };
}
...
const VITE_SERVER_CONFIG = (config) => {
  ...
  return {
    ...
    plugins: [vue__default(), tez(), tezGenBundle(), tezPWA()]
  };
};
